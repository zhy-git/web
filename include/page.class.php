...
class Page
{
	...
	function GetList()
	{
		global $cfg_isreurl,$keyword,$cfg_mobile;
		...
		else
		{
			...
			if($query_str[0] != '')
			{
				...
				foreach($query_str as $k)
				{
					...
					if(strstr($query_str_arr[0],'page') == '')
					{
						...
						{
							$query_strs .= $cfg_mobile == 'Y' ? $query_str_arr[0].'='.$query_str_arr[1].'&' : '-'.$query_str_arr[1];
						}
						else
						{
			...
			if($cfg_isreurl == 'Y' && !isset($keyword))
			{
				...
				//获取除页码以外的参数
				    ...
				    if($cfg_mobile=='Y'){}else{$nowurl      = $request_str.ltrim($nowurl,'?');}
			}
			...
			if($this->page > 1)
			{
				...
				if($cfg_isreurl == 'Y' && !isset($keyword))
				{
					$pagetxt .= $cfg_mobile=='Y'?'<a href="'.$nowurl.'page=1" title="第一页">&lt;&lt;</a>':'<a href="'.$nowurl.'-1.html" title="第一页">&lt;&lt;</a>';
					$pagetxt .= $cfg_mobile=='Y'?'<a href="'.$nowurl.'page='.$previous.'" title="上一页">&lt;</a>':'<a href="'.$nowurl.'-'.$previous.'.html" title="上一页">&lt;</a>';
				}
				else
				{
			...
			else
			{
				...
				for($i=$m; $i<=$b; $i++)
				{
					...
					else
					{
						...
						if($cfg_isreurl == 'Y' && !isset($keyword))
						{
							$pagetxt .= $cfg_mobile=='Y'?'<a href="'.$nowurl.'page='.$i.'" class="num" title="第 '.$i.' 页">'.$i.'</a>':'<a href="'.$nowurl.'-'.$i.'.html" class="num" title="第 '.$i.' 页">'.$i.'</a>';
						}
						else
						{
							$pagetxt .= '<a href="'.$nowurl.'page='.$i.'" class="num" title="第 '.$i.' 页">'.$i.'</a>';
			...
			if($this->page < $this->totalpage)
			{
				...
				if($cfg_isreurl == 'Y' &&
				   !isset($keyword))
				{
					$pagetxt .= $cfg_mobile=='Y'?'<a href="'.$nowurl.'page='.$next.'" title="下一页">&gt;</a>':'<a href="'.$nowurl.'-'.$next.'.html" title="下一页">&gt;</a>';
					$pagetxt .= $cfg_mobile=='Y'?'<a href="'.$nowurl.'page='.$this->totalpage.'" title="最后一页">&gt;&gt;</a>':'<a href="'.$nowurl.'-'.$this->totalpage.'.html" title="最后一页">&gt;&gt;</a>';
				}
				else
				{
